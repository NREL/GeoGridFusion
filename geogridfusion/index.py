...
def unform_coordinates_array(resolution: str) -> np.ndarray:
    ...
    resolution: int, str
        ...
        resolution of grid in km.

        Exmple: 4km grid
            ``resolution=4`` or `resolution='4'`
    """

    grid_points_fn = f"{resolution}km-points.npz"

    with open(
        os.path.join(geogridfusion.TREE_BINARIES_DIR, grid_points_fn), "rb"
    ) as fp:
        npz_file = np.load(fp)
...
def generate_spherical_grid_fixed(distance_km, radius=6371):
    ...
    for lat in latitudes:
        ...
        circumference = (
            2 * np.pi * radius * np.cos(lat)
        )  # Circumference of latitude circle
        num_points = max(
            1, int(circumference / distance_km)
