...
def store_single(conn: connection, weather_df: pd.DataFrame, meta: dict, tmy: bool, source_name: str = None, coerce_year: int=1979) -> None:
    ...
    if source_name is None:
        raise ValueError("Missing source name: source must be in 'meta' or manually provided in 'source_name'")
...
def load_many(
    ...
    if any(dfs[0][1].shape != df.shape for _id, df in dfs[1:]): # generator expr 
        raise ValueError(f"""mismatched dimensions... (expand error for usefulness)""")
